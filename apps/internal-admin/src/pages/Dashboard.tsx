...
const Dashboard: React.FC = () => {
  ...
  const [health, setHealth] = useState<
    Record<
      string,
      { loading: boolean; status: string | null; latency: number | null }
    >
  >({
  ...
  const fetchAllHealth = () => {
    fetchHealthForItem(
      "database",
      "http://localhost:3000/api/super-admin/dashboard/health/database",
    );
    fetchHealthForItem(
      "twilio",
      "http://localhost:3000/api/super-admin/dashboard/health/twilio",
    );
    fetchHealthForItem(
      "sendgrid",
      "http://localhost:3000/api/super-admin/dashboard/health/sendgrid",
    );
    fetchHealthForItem(
      "vnpay",
      "http://localhost:3000/api/super-admin/dashboard/health/vnpay",
    );
    fetchHealthForItem(
      "ai",
      "http://localhost:3000/api/super-admin/dashboard/health/ai",
    );
  };
  ...
  const [actions, setActions] = useState([
    {
      id: 1,
      type: "Tenant",
      message: "Salon 'HairSalon Việt' đăng ký dùng thử gói Basic",
      time: "10 phút trước",
      user: "Hệ thống",
    },
    {
      id: 2,
      type: "Billing",
      message: "Duyệt hóa đơn gia hạn 6 tháng cho 'BarberShop Q1'",
      time: "42 phút trước",
      user: "Hoàng Admin",
    },
    {
      id: 3,
      type: "Security",
      message: "Yêu cầu thay đổi mật khẩu từ chủ salon 'TinaSpa'",
      time: "2 giờ trước",
      user: "Hệ thống",
    },
    {
      id: 4,
      type: "API",
      message: "Cập nhật cấu hình tích hợp SMS Brandname cho 'VinaBarber'",
      time: "5 giờ trước",
      user: "Thế Anh (Manager)",
    },
  ]);
  ...
  const fetchDashboardData = async (showLoading = true) => {
    try {
      ...
      const statsRes = await fetch(
        "http://localhost:3000/api/super-admin/dashboard/stats",
      );
  ...
  const stats = [
    ...
    },
    {
      ...
      growth: data?.stats
        ? `${data.stats.activeTenants} đang hoạt động / ${data.stats.suspendedTenants} tạm ngưng`
        : "0 đang hoạt động / 0 tạm ngưng",
      icon: Users,
      color: "var(--color-success)",
    ...
    },
    {
      ...
      value: data?.stats
        ? `${data.stats.totalBookings.toLocaleString("vi-VN")} lượt`
        : "0 lượt",
    ...
    >
        ...
        >
          ...
          >
            <h3 className="card-title" style={{ margin: 0 }}>
              Biểu đồ Tăng Trưởng Doanh Thu (MRR)
            </h3>
            <span
              style={{
                fontSize: "12px",
            ...
            >
              ...
              >
                ...
                <circle
                  ...
                  strokeDashoffset={`-${freePct + basicPct}`}
                />
              </svg>
            </div>
      ...
      >
        ...
        >
          ...
          >
            <h3 className="card-title" style={{ margin: 0 }}>
              Trạng Thái Kết Nối API & Đối Tác
            </h3>
            <button
            ...
            >
              <RefreshCw
                size={12}
                className={isAnyHealthLoading ? "animate-spin" : ""}
              />{" "}
              Làm mới
            </button>
          </div>
          ...
          >
            {(() => {
              const services = [
                {
                  key: "database",
                  name: "Database PostgreSQL (Supabase Connection)",
                  isDb: true,
                },
                { key: "twilio", name: "Twilio SMS Gateway API", isDb: false },
                {
                  key: "sendgrid",
                  name: "SendGrid Email API Service",
                  isDb: false,
                },
                {
                  key: "vnpay",
                  name: "VNPAY Payment IPN Gateway",
                  isDb: false,
                },
                {
                  key: "ai",
                  name: "DeepSeek AI Mapping API (Core Engine)",
                  isDb: false,
                },
              ];
              ...
              return services.map((srv) => {
                ...
                const metric = item.loading
                  ...
                  : {
                      ...getMetricDetails(item.status, item.latency, srv.isDb),
                      isLoading: false,
                    };
                  ...
                  <div key={srv.key}>
                    ...
                    >
                      <div
                        style={{
                          height: "6px",
                          backgroundColor: metric.isLoading
                            ? "var(--color-primary-light)"
                            : metric.color,
                          borderRadius: "var(--radius-full)",
                          width: `${metric.pct}%`,
