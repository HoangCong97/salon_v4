...
const Subscriptions: React.FC = () => {
  ...
  const [viewingInvoice, setViewingInvoice] = useState<BillingInvoice | null>(
    null,
  );
  ...
  const fetchData = async (showLoading = true) => {
    try {
      ...
      const invoicesRes = await fetch(
        "http://localhost:3000/api/super-admin/invoices",
      );
  ...
  const approveInvoice = async (id: string) => {
    try {
      const res = await fetch(
        `http://localhost:3000/api/super-admin/invoices/${id}/approve`,
        {
          method: "PUT",
        },
      );
      if (res.ok) {
        ...
        if (viewingInvoice && viewingInvoice.id === id) {
          setViewingInvoice((prev) =>
            prev ? { ...prev, status: "PAID" as const } : null,
          );
        }
      }
    } catch (error) {
  ...
  const handleSavePlan = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:3000/api/super-admin/plans/${editingPlan.id}`,
        {
          method: "PUT",
          headers: {
        ...
        <div>
          ...
          >
            ...
            <h3 className="card-title" style={{ margin: 0 }}>
              Cấu Hình Gói Subscription (Quotas)
            </h3>
          </div>
              ...
              >
                ...
                >
                  ...
                  >
                    {plan.price === 0
                      ? "Miễn phí"
                      : formatCurrencyVND(plan.price).replace("đ", "")}
                  </span>
                  {plan.price > 0 && (
                    <span
                ...
                >
                  <div>
                    <strong>Chi nhánh tối đa:</strong>{" "}
                    {plan.maxBranches === -1
                      ? "Không giới hạn"
                      : `${plan.maxBranches} chi nhánh`}
                  </div>
                  <div>
                    <strong>Thợ làm tối đa:</strong>{" "}
                    {plan.maxStaff === -1
                      ? "Không giới hạn"
          ...
          >
            ...
            >
              <h3 style={{ margin: 0, fontSize: "16px", fontWeight: 600 }}>
                Cấu hình Quotas & Tính năng: {editingPlan.name}
              </h3>
              <button
            ...
            >
              ...
              <div className="form-group">
                <label className="form-label">
                  Giới Hạn Chi Nhánh (Branches limit)
                </label>
                <select
                  className="form-input"
                  ...
                  onChange={(e) => {
                    ...
                    if (val === "custom") {
                      const currentNum =
                        editForm.maxBranches > 0 ? editForm.maxBranches : 1;
                      setEditForm((prev) => ({
                        ...prev,
                        maxBranches: currentNum,
                      }));
                    } else {
                      setEditForm((prev) => ({
                        ...prev,
                        maxBranches: parseInt(val),
                      }));
                    }
                  }}
                >
                ...
                {branchSelectValue === "custom" && (
                  <input
                    ...
                    onChange={(e) =>
                      setEditForm((prev) => ({
                        ...prev,
                        maxBranches: Math.max(1, parseInt(e.target.value) || 1),
                      }))
                    }
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (val === "custom") {
                      const currentNum =
                        editForm.maxStaff > 0 ? editForm.maxStaff : 5;
                      setEditForm((prev) => ({
                        ...prev,
                        maxStaff: currentNum,
                      }));
                ...
                {staffSelectValue === "custom" && (
                  <input
                    ...
                    onChange={(e) =>
                      setEditForm((prev) => ({
                        ...prev,
                        maxStaff: Math.max(1, parseInt(e.target.value) || 1),
                      }))
                    }
          ...
          >
            ...
            >
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                ...
                <span>
                  {new Date(viewingInvoice.date).toLocaleString("vi-VN")}
                </span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ color: "var(--text-secondary)" }}>
              ...
              >
                ...
                >
                  {formatCurrencyVND(viewingInvoice.amount)}
                </strong>
              </div>
              <div
                style={{
