  ...
  const loadProfile = async (phoneNum: string) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/customers/profile?phone=${phoneNum}&tenantId=${tenantId}`,
      );
      if (response.ok) {
        ...
        setErrorMsg(
          errData.message ||
            "Không tìm thấy khách hàng. Vui lòng kiểm tra lại số điện thoại.",
        );
      ...
      {!profile && (
        ...
        >
          <div className="card" style={{ padding: "24px 20px" }}>
            ...
            >
              Nhập số điện thoại đặt lịch để xem lịch sử hẹn, tích lũy điểm
              thưởng và các gói liệu trình của bạn.
            </p>
            ...
            {profile.activePackages.length > 0 ? (
              profile.activePackages.map((pkg) => (
                ...
                >
                  ...
                  >
                    <span>
                      Đã dùng: {pkg.usedQuantity} / Tổng: {pkg.quantity}
                    </span>
                ...
                >
                  ...
                  {item.status === "COMPLETED" && (
                    ...
                    >
                      ...
                      >
                        {item.hasReview
                          ? `Bạn đã đánh giá: ${"★".repeat(item.reviewRating || 5)}`
                          : "Chưa có đánh giá"}
                      </span>
                      {!item.hasReview && (
                        <Link
                          href={`/reviews/${item.id}?phone=${profile.phone}`}
