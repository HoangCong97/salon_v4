...
export function ServiceModal({
  ...
  const [form, setForm] = useState({
    ...
    startTime:
      ...
      state.prefill?.startTime ??
      getClosestTimeOption(START_HOUR, END_HOUR),
  ...
  const selSvcs = serviceList.filter((s) =>
  ...
  const filteredSvcs = serviceList.filter((s) =>
  ...
  const validate = () => {
    ...
    if (form.selectedServiceIds.length === 0)
      e.services = "Chọn ít nhất 1 dịch vụ";
    setErrors(e);
        ...
        <div className={styles.modalHeader}>
          ...
          <div>
            ...
            <p className={styles.modalHeaderSub}>
              {isEdit
                ? "Cập nhật thông tin"
                : "Mỗi dịch vụ = 1 thẻ độc lập trên lịch"}
            </p>
          </div>
        ...
        <div className={styles.modalBody}>
          ...
          <div className={styles.formGrid2}>
            <div>
              ...
              {errors.customer && (
                <p className={styles.errorText}>{errors.customer}</p>
              )}
            </div>
            <div>
            ...
            <div className={styles.servicesListContainer}>
              {filteredSvcs.map((sv) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`${styles.serviceCheckbox} ${sel ? styles.serviceCheckboxSelected : ""}`}
                    >
                      {sel && <CheckCircle2 size={9} color="white" />}
                    </div>
                    <span className={styles.serviceItemName}>{sv.name}</span>
                    <span className={styles.serviceItemDuration}>
                      {sv.duration}p
                    </span>
                    <span className={styles.serviceItemPrice}>
                      {fmtCurrency(sv.price)}
                    </span>
                  </div>
                );
              })}
          ...
          {selSvcs.length > 0 && (
            <div className={styles.summaryGradientBox}>
              ...
              <div className={styles.summaryItem}>
                ...
                <div className={styles.summaryValuePurple}>
              ...
              {!isEdit && selSvcs.length > 1 && (
                <>
                  ...
                  <div className={styles.summaryItem}>
                    ...
                    <div className={styles.summaryValueViolet}>
                      {selSvcs.length}
                    </div>
                  </div>
                </>
              ...
              <div className={styles.statusButtonsContainer}>
                {(Object.keys(STATUS_CFG) as AppointmentStatus[]).map((s) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className={styles.statusModalDot}
        ...
        <div className={styles.modalFooter}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button onClick={handleSave} className={styles.saveBtn}>
              {isEdit
                ? "Lưu thay đổi"
                : `Thêm ${selSvcs.length > 1 ? selSvcs.length + " dịch vụ" : "lịch hẹn"}`}
            </button>
          </div>
        </div>
      </div>
