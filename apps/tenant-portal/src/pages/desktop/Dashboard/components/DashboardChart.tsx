  ...
  return (
    <div className={styles.dashboardSplitSection}>
      ...
      <div className={styles.splitLeftCard}>
        ...
        <p className={styles.chartSub} style={{ margin: "4px 0 12px 0", fontSize: "11px", color: "var(--text-muted)", fontStyle: "italic" }}>
        ...
        <div className={styles.horizontalChartContainer}>
          {monthlyTrends.map((item) => {
            ...
            const totalBarWidth = (item.totalPrice / maxMonthlyVal) * 80; // max 80% width

            // Calculate percentage of final vs discount within the total price bar
            const finalPercent = item.totalPrice > 0 ? (item.finalAmount / item.totalPrice) * 100 : 0;
            const discountPercent = item.totalPrice > 0 ? (item.discountAmount / item.totalPrice) * 100 : 0;

            return (
              <div
                key={item.month}
                className={`${styles.horizontalBarRow} ${isActive ? styles.activeMonthRow : ""
                  }`}
              ...
              >
                ...
                <div className={styles.monthLabelCol}>
                  <span className={styles.singleLineMonth}>{formatMonthLabel(item.month)}</span>
                </div>
                ...
                <div className={styles.barsCol}>
                  {item.totalPrice > 0 ? (
                    ...
                    >
                      ...
                      {(finalPercent <= 25 || (item.discountAmount > 0 && discountPercent <= 20)) && (
                    ...
                    <span className={styles.noRevenueText}>Không phát sinh</span>
                  )}
                </div>
              </div>
            );
          ...
          <table className={styles.excelDailyTable}>
            <thead>
              <tr>
                ...
                <th style={{ width: "85px", textAlign: "right" }}>Giá dịch vụ</th>
                <th style={{ width: "90px", textAlign: "right" }}>Thành tiền</th>
              </tr>
            </thead>
            <tbody>
              {dailyRevenues.map((d, index) => (
                <tr
                  ...
                  data-day={d.day}
                >
                  <td style={{ fontWeight: "500", textAlign: "center" }}>{formatDayLabel(d.dateRaw)}</td>
                          ...
                          <div key={item.id} className={styles.subTableRow}>
                            <span className={styles.subItemName}>
                              ...
                              <span className={styles.itemTypeTag}>
                                {item.itemType === "SERVICE" ? "Dịch vụ" : item.itemType === "PRODUCT" ? "Sản phẩm" : "Gói"}
                              </span>
                            </span>
