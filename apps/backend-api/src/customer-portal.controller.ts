...
export class CustomerPortalController {
  ...
  async getTimeSlots(
    ...
    try {
      if (!branchId || !dateStr || !serviceId) {
        throw new HttpException(
          "branchId, date, and serviceId are required",
      ...
      const results = timeSlots.map((slotTime) => {
        ...
        const isOverlapping = (bStart: Date, bDur: number) => {
          ...
          const bookingEnd = new Date(
            bookingStart.getTime() + bDur * 60 * 1000,
          );
          return (
        ...
        if (staffId && staffId !== "any") {
          ...
          const stylistBookings = activeBookings.filter(
            (b) => b.staffId === staffId,
          );
      ...
      throw new HttpException(
        `Failed to query slots: ${(error as any).message}`,
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async createBooking(
    ...
    try {
      ...
      return {
        ...
        staffName:
          booking.bookingDetails[0].staff?.name || "Ngẫu nhiên/Chưa gán",
  ...
  async getCustomerProfile(
    ...
    try {
      if (!phone || !tenantId) {
        throw new HttpException(
          "Phone and tenantId are required",
          HttpStatus.BAD_REQUEST,
        );
      }
      ...
      // 2. Compute stats
      const totalPoints = customer.points.reduce(
        (acc: number, p: any) => acc + (p.point || 0),
        0,
      );
  ...
  async leaveReview(
    ...
    try {
      ...
      if (!booking.customer || booking.customer.phone !== phone) {
        throw new HttpException(
          "Unauthorized booking ownership",
      ...
      if (error instanceof HttpException) throw error;
      throw new HttpException(
        `Failed to create review: ${(error as any).message}`,
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async getLatestReviews(
    ...
    try {
      ...
      throw new HttpException(
        `Failed to fetch reviews: ${(error as any).message}`,
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
